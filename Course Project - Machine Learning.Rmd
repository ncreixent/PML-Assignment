...
Our approach to generate the final predicition will be to ensemble three classification algorithms: random forest, linear discriminant analysis and naive bayes. We will train the algorithms in the training set and get an individual out of sample measure using the test subset. Also, using the test subset we will calibrate the ensemble, which will be generated using a random forest that will feed from the three individual algorithms. Finally, we will get an OOS estimation by testing the ensembled model on the validation set.

First we splitted the training set into three subsets (train, test and validation) to allow for cross validation.
...
Considering that after the cleanup we still have 52 variables, we evaluated whether the variables are correlated. As can be seen in the following level plot, the variables that are significantly correlated. This indicates that we might benefit from preprocessing via principal component analysis.
...
In short, we obtained three models with reasonable OO accuracy:
